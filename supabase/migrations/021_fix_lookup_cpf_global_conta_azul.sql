-- ============================================================
-- Ajuste na busca por CPF: Conta Azul
-- 1) Comparar contato_cpf normalizando dígitos (com ou sem formatação)
-- 2) Comparar nomes sem case-sensitive (LOWER/TRIM) para contato_nome e fornecedor_nome
-- ============================================================
...
BEGIN
    ...
    FROM user_profiles p WHERE REGEXP_REPLACE(COALESCE(p.cpf, ''), '[^0-9]', '', 'g') = clean LIMIT 1;
    ...
    WHERE REGEXP_REPLACE(COALESCE(up.cpf, ''), '[^0-9]', '', 'g') = clean;

    -- Colaborador
    SELECT to_jsonb(c.*) INTO v_collaborator
    FROM crm_collaborators c
    WHERE REGEXP_REPLACE(COALESCE(c.cpf, ''), '[^0-9]', '', 'g') = clean
    LIMIT 1;

    -- Instrutor
    SELECT to_jsonb(t.*) INTO v_instructor
    FROM crm_teachers t
    WHERE REGEXP_REPLACE(COALESCE(t.cpf, ''), '[^0-9]', '', 'g') = clean
    LIMIT 1;

    -- Aluno (cadastro central)
    SELECT to_jsonb(a.*) INTO v_student
    FROM crm_alunos a WHERE REGEXP_REPLACE(COALESCE(a.cpf, ''), '[^0-9]', '', 'g') = clean
    LIMIT 1;
    ...
    WHERE REGEXP_REPLACE(COALESCE(a.cpf, ''), '[^0-9]', '', 'g') = clean;

    -- Deals (compras/vendas)
    SELECT COALESCE(jsonb_agg(to_jsonb(d.*) ORDER BY d.created_at DESC), '[]'::JSONB) INTO v_deals
    FROM crm_deals d
    WHERE REGEXP_REPLACE(COALESCE(d.cpf, ''), '[^0-9]', '', 'g') = clean;

    -- Studio Parceiro
    SELECT to_jsonb(s.*) INTO v_partner_studio
    FROM crm_partner_studios s
    WHERE REGEXP_REPLACE(COALESCE(s.cpf, ''), '[^0-9]', '', 'g') = clean
    LIMIT 1;

    -- Franquia
    SELECT to_jsonb(f.*) INTO v_franchise
    FROM crm_franchises f
    WHERE REGEXP_REPLACE(COALESCE(f.cpf, ''), '[^0-9]', '', 'g') = clean
    LIMIT 1;

    -- Coletar nomes conhecidos da pessoa para fallback no Conta Azul
    SELECT array_agg(DISTINCT nm) INTO v_known_names
    FROM (
        SELECT TRIM(full_name) AS nm FROM user_profiles WHERE REGEXP_REPLACE(COALESCE(cpf, ''), '[^0-9]', '', 'g') = clean AND full_name IS NOT NULL AND TRIM(full_name) <> ''
        UNION
        SELECT TRIM(full_name) FROM crm_collaborators WHERE REGEXP_REPLACE(COALESCE(cpf, ''), '[^0-9]', '', 'g') = clean AND full_name IS NOT NULL AND TRIM(full_name) <> ''
        UNION
        SELECT TRIM(full_name) FROM crm_teachers WHERE REGEXP_REPLACE(COALESCE(cpf, ''), '[^0-9]', '', 'g') = clean AND full_name IS NOT NULL AND TRIM(full_name) <> ''
        UNION
        SELECT TRIM(full_name) FROM crm_alunos WHERE REGEXP_REPLACE(COALESCE(cpf, ''), '[^0-9]', '', 'g') = clean AND full_name IS NOT NULL AND TRIM(full_name) <> ''
        UNION
        SELECT TRIM(company_name) FROM crm_deals WHERE REGEXP_REPLACE(COALESCE(cpf, ''), '[^0-9]', '', 'g') = clean AND company_name IS NOT NULL AND TRIM(company_name) <> ''
        UNION
        SELECT TRIM(contact_name) FROM crm_deals WHERE REGEXP_REPLACE(COALESCE(cpf, ''), '[^0-9]', '', 'g') = clean AND contact_name IS NOT NULL AND TRIM(contact_name) <> ''
    ) names;

    -- Conta Azul - Contas a Receber: CPF normalizado (com ou sem pontuação) OU nome com LOWER/TRIM
    SELECT COALESCE(jsonb_agg(to_jsonb(cr.*) ORDER BY cr.data_vencimento DESC NULLS LAST), '[]'::JSONB)
    INTO v_ca_receber
    FROM conta_azul_contas_receber cr
    WHERE (
        REGEXP_REPLACE(COALESCE(cr.contato_cpf, ''), '[^0-9]', '', 'g') = clean
        OR (
    ...
    -- Conta Azul - Contas a Pagar: CPF normalizado OU fornecedor_nome com LOWER/TRIM
    SELECT COALESCE(jsonb_agg(to_jsonb(cp.*) ORDER BY cp.data_vencimento DESC NULLS LAST), '[]'::JSONB)
    INTO v_ca_pagar
    FROM conta_azul_contas_pagar cp
    WHERE (
        REGEXP_REPLACE(COALESCE(cp.contato_cpf, ''), '[^0-9]', '', 'g') = clean
        OR (
            v_known_names IS NOT NULL
            AND array_length(v_known_names, 1) > 0
    ...
    WHERE REGEXP_REPLACE(COALESCE(po.student_cpf, ''), '[^0-9]', '', 'g') = clean;
