...
import { LandingPage, LandingPageSection, ElementStyles } from '../types';
import { 
...
export const LandingPagePublicViewer: React.FC<LandingPagePublicViewerProps> = ({ landingPage }) => {
  ...
  const renderSection = (section: LandingPageSection) => {
    switch (section.type) {
              ...
              <div className="space-y-8">
                ...
                {hero.subheadline && (
                    <p 
                        ...
                        className="text-xl text-slate-500 max-w-2xl mx-auto lg:mx-0 leading-relaxed font-medium"
                    >
                    {hero.subheadline}
                    </p>
                )}
          ...
          <section key={section.id} className="py-24 px-6 bg-white">
            <div className="max-w-7xl mx-auto">
              <div className="text-center mb-16">
                <h2 
                    style={getStyles(section, 'mainTitle')}
                    className="text-3xl font-black text-slate-900 tracking-tight mb-4"
                >
                    {section.content.mainTitle || 'Por que escolher esta formação?'}
                </h2>
          ...
          <section key={section.id} id="pricing" className="py-24 px-6 bg-slate-900 text-white overflow-hidden relative">
            ...
            <div className="max-w-4xl mx-auto text-center relative z-10">
              ...
              <div className="bg-white rounded-[3rem] p-10 md:p-16 text-slate-900 shadow-2xl relative">
                ...
                <a 
                    href={pricing.ctaUrl || "#"}
                    className="block w-full bg-orange-600 hover:bg-orange-700 text-white py-6 rounded-2xl font-black text-xl uppercase tracking-widest shadow-2xl shadow-orange-600/30 transition-all active:scale-95 mb-6 text-center"
                >
                  {pricing.ctaText}
                </a>
      ...
      case 'form':
        return (
            <section key={section.id} className="py-24 px-6 bg-white border-y border-slate-100 flex justify-center">
                <div className="max-w-2xl w-full">
                    {formModel ? (
                        <div className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100 shadow-inner">
                            <h3 className="text-2xl font-black text-slate-800 text-center mb-8">{section.content.title}</h3>
                            <FormViewer form={formModel} isPublic={true} />
                        </div>
                    ) : (
                        <div className="p-12 bg-slate-50 rounded-[2.5rem] border-2 border-dashed border-slate-200 text-center text-slate-400">
                            <FormInput size={48} className="mx-auto mb-4 opacity-10" />
                            <p className="font-bold">Formulário não vinculado ou inexistente.</p>
                        </div>
                    )}
                </div>
            </section>
...
const FormInput = ({ size, className }: { size: number, className?: string }) => (
    <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><rect width="18" height="18" x="3" y="3" rx="2" ry="2"/><path d="M7 7h10"/><path d="M7 12h10"/><path d="M7 17h10"/></svg>
);
