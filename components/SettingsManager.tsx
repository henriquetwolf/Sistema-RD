    ...
    <div className="animate-in fade-in space-y-8 pb-20">
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex bg-slate-100 p-1 rounded-lg">
            <button onClick={() => setActiveTab('visual')} className={clsx("px-4 py-2 text-xs font-bold rounded-md", activeTab === 'visual' ? "bg-white shadow" : "text-slate-500")}>Visual</button>
      ...
      {activeTab === 'database' && (
          <div className="bg-white rounded-xl border border-slate-200 p-8 space-y-6">
              <div className="flex items-center gap-3"><Database className="text-amber-600"/><h3 className="text-lg font-bold">Manutenção (V22)</h3></div>
              <p className="text-sm text-slate-500 font-bold text-red-600 flex items-center gap-2"><AlertTriangle size={16}/> Rode este script para habilitar o novo Sistema de Chamados.</p>
              {!showSql ? <button onClick={() => setShowSql(true)} className="w-full py-3 bg-slate-900 text-slate-100 rounded-lg font-mono text-sm">Gerar Script V22</button> : (
