...
import { 
  ...
  GraduationCap, Inbox, AlertTriangle, Trash2, CheckCircle, X
} from 'lucide-react';
...
export const SupportChannel: React.FC<SupportChannelProps> = ({ 
  ...
  useEffect(() => {
    if (selectedTicket) {
      ...
      // Polling básico para simular tempo real
      const interval = setInterval(() => fetchMessages(selectedTicket.id, true), 8000);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      await appBackend.addTicketMessage(
        selectedTicket.id, 
        replyText, 
        userName || (isAdmin ? 'Suporte VOLL' : 'Usuário'),
        isAdmin ? 'agent' : 'user'
      );
      setReplyText('');
  ...
  const getStatusBadge = (status: Ticket['status']) => {
    const config = {
      ...
      closed: { color: 'bg-slate-100 text-slate-500', label: 'Encerrado' }
    };
        ...
        <div className="p-5 border-b border-slate-100 bg-slate-50/50 space-y-4">
            <div className="flex items-center justify-between">
                <h2 className="text-lg font-black text-slate-800 flex items-center gap-2">
                    <LifeBuoy className="text-indigo-600" /> Canal de Suporte
                </h2>
                {!isAdmin && (
      ...
      <div className="flex-1 flex flex-col bg-slate-50 relative min-w-0">
          {selectedTicket ? (
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar bg-slate-50">
                    {loadingMessages ? (
                        ...
                        <div className="flex flex-col gap-6">
                            <div className="text-center"><span className="text-[10px] font-bold text-slate-300 uppercase tracking-[0.3em]">Chamado Aberto em {new Date(selectedTicket.createdAt).toLocaleString()}</span></div>
                            
                            {messages.map((msg) => {
                                const isMe = isAdmin ? msg.senderType === 'agent' : msg.senderType === 'user';
                                return (
                ...
                <div className="bg-white p-5 border-t border-slate-200">
                    <form onSubmit={handleSendMessage} className="flex gap-4">
                        ...
                        <button 
                            ...
                            className="bg-indigo-600 hover:bg-indigo-700 text-white p-4 rounded-2xl font-black text-sm uppercase shadow-lg shadow-indigo-600/20 transition-all active:scale-95 disabled:opacity-50"
                        >
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-slate-400 p-12 text-center animate-in fade-in zoom-in-95">
                ...
                <h3 className="text-xl font-black text-slate-800 mb-2">Central de Atendimento VOLL</h3>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
              <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg animate-in zoom-in-95 flex flex-col">
                  <div className="px-10 py-8 border-b border-slate-100 flex justify-between items-center bg-slate-50 rounded-t-[2.5rem]">
                      ...
                      {/* Fix for: Cannot find name 'X'. Added to imports above. */}
                  ...
                  <form onSubmit={handleCreateTicket} className="p-10 space-y-6">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                          <div>
                              ...
                              <select className="w-full px-4 py-3 border-2 border-slate-100 bg-slate-50 rounded-2xl text-sm font-bold outline-none" value={newTicketForm.category} onChange={e => setNewTicketForm({...newTicketForm, category: e.target.value})}>
