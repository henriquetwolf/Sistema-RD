...
export const LandingPagePublicViewer: React.FC<LandingPagePublicViewerProps> = ({ landingPage }) => {
  ...
  const renderDynamicContent = (section: LandingPageSection, key: string, field: any) => {
      ...
      // Se for imagem
      if (field.type === 'image') {
          return <img key={key} src={value} style={styles} className="rounded-2xl shadow-lg mx-auto block" alt="Conteúdo visual" />;
      }
      
      // Se for vídeo
      if (field.type === 'video') {
          return (
            <div key={key} style={styles} className="mx-auto w-full">
                <div className="aspect-video rounded-3xl overflow-hidden shadow-2xl bg-black" dangerouslySetInnerHTML={{ __html: value }}></div>
            </div>
          );
      }
      ...
      // Fallback para campos de texto dinâmicos (keys que contém _)
      if (key.includes('_') || !field.type) {
          return <div key={key} style={styles} className="whitespace-pre-wrap">{value}</div>;
      }
  ...
  const renderSection = (section: LandingPageSection) => {
    ...
    switch (section.type) {
      case 'hero':
        return (
          <header key={section.id} id={section.id} className="pt-32 pb-24 px-6 bg-gradient-to-br from-slate-50 to-white relative overflow-hidden" style={{ backgroundColor: content.theme.bg_color }}>
            <div className="absolute top-0 right-0 -mt-20 -mr-20 w-[600px] h-[600px] bg-indigo-50/50 rounded-full blur-[100px]"></div>
            <div className="max-w-6xl mx-auto text-center relative z-10">
              <div className="inline-flex items-center gap-2 bg-indigo-100 text-indigo-700 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-[0.2em] mb-8">
                  <Award size={14}/> {content.theme.brand_name} • Premium
              </div>
      ...
      case 'pain':
        return (
          <section key={section.id} id={section.id} className="py-24 px-6 bg-slate-900 text-white relative">
      ...
      case 'method':
        return (
            <section key={section.id} id={section.id} className="py-24 px-6 bg-white">
                ...
                <div className="max-w-5xl mx-auto mt-12">{renderInnerContent()}</div>
            </section>
        );

      case 'benefits':
        return (
          <section key={section.id} id={section.id} className="py-24 px-6 bg-slate-50">
      ...
      case 'bonuses':
        return (
            <section key={section.id} id={section.id} className="py-24 px-6 bg-indigo-900 text-white">
                <div className="max-w-6xl mx-auto text-center">
                    <div className="mb-16">
                        ...
                        {c.items?.map((item: any, idx: number) => (
                            <div key={idx} className="p-8 rounded-[2.5rem] bg-slate-50 border border-slate-100 flex flex-col items-start text-left">
                                ...
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-full bg-indigo-100 flex items-center justify-center font-bold text-indigo-600">{(item.author?.value || item.author || 'A').charAt(0)}</div>
                                    <div>
        ...
        return (
          <section key={section.id} id={section.id} className="py-12 px-6">
            <div className="max-w-5xl mx-auto text-center">
              {c.headline && <h2 className="text-3xl font-black mb-8" style={getStyles(section, 'headline')}>{c.headline.value}</h2>}
              {renderInnerContent()}
            </div>
          </section>
        );
