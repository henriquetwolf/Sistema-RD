...
export const LandingPageManager: React.FC<LandingPageManagerProps> = ({ onBack }) => {
  ...
  const [creationStep, setCreationStep] = useState<'choice' | 'form'>('form');
  const [creationMode, setCreationMode] = useState<'standard' | 'prompt'>('prompt');
            ...
            <div className="p-10 overflow-y-auto custom-scrollbar flex-1">
               {!currentDraft ? (
                  <div className="space-y-8 animate-in slide-in-from-right-4 duration-300">
                     <div className="flex items-center gap-4 mb-4"><button onClick={() => setShowModal(false)} className="p-2 hover:bg-slate-100 rounded-full text-slate-500 transition-colors"><ArrowLeft size={18}/></button><h4 className="text-sm font-black text-indigo-600 uppercase tracking-widest">Geração de Código HTML Personalizado</h4></div>
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
