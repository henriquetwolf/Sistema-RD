...
export const ContaAzulManager: React.FC = () => {
  ...
  // Busca TODOS os dados da tabela em lotes de 1000 (Bypass de limite da API)
  const fetchAllData = async () => {
    ...
    try {
      // 1. Identifica qual tabela existe
      ...
      while (hasMore) {
        ...
        if (batch && batch.length > 0) {
          ...
          setLoadProgress(allRecords.length);
        ...
        // Pequena pausa para não travar a UI em tabelas massivas
        if (from % 5000 === 0) await new Promise(r => setTimeout(r, 10));
      }
  ...
  const filteredData = useMemo(() => {
    return data.filter(item => {
      // 1. Filtro de Data (Suporta formatos variados: Texto DD/MM/YYYY ou ISO)
      ...
      <div className="flex-1 flex flex-col min-h-0">
        {viewMode === 'dashboard' ? (
          <div className="space-y-6 animate-in slide-in-from-bottom-4">
            ...
            <div className="bg-white p-8 rounded-[2.5rem] border-2 border-blue-50 shadow-xl space-y-6">
              <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                 <div className="flex items-center gap-3">
                    ...
                    <h3 className="font-black text-slate-800 uppercase tracking-tight text-lg">Central de Dados Conta Azul</h3>
                 </div>
                 {isLoading && (
                     <div className="flex items-center gap-3 text-blue-600 bg-blue-50 px-4 py-2 rounded-full border border-blue-100 animate-pulse">
                         <Loader2 size={16} className="animate-spin" />
                         <span className="text-[10px] font-black uppercase tracking-widest">Sincronizando {loadProgress.toLocaleString()} registros...</span>
                     </div>
                 )}
              </div>
              ...
              <div className="flex items-center justify-between pt-4">
                <div className="flex items-center gap-2 text-slate-400 text-[10px] font-medium max-w-md">
                    ...
                    <span>O sistema carrega <strong>todos</strong> os registros do banco e aplica os filtros acima instantaneamente. Clique em "Sincronizar" para atualizar.</span>
                </div>
                <div className="flex gap-3">
                  <button onClick={handleResetFilters} className="px-6 py-3 text-slate-400 hover:text-red-500 font-bold text-xs uppercase tracking-widest transition-all flex items-center gap-2"><Eraser size={16}/> Resetar Filtros</button>
                  <button 
                  ...
                  >
                    ...
                    {data.length > 0 ? 'Sincronizar Banco de Dados' : 'Baixar Dados do Banco'}
                  </button>
                </div>
              </div>
            </div>
            ...
            {data.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in zoom-in-95">
                    <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm relative overflow-hidden group">
                        ...
                        <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Volume de {filteredData.length.toLocaleString()} itens</p>
                        <h3 className="text-4xl font-black text-slate-800 tracking-tight">{formatCurrency(stats.totalValue)}</h3>
                        <p className="text-[10px] text-slate-400 mt-2 font-bold uppercase">Total bruto filtrado</p>
                    </div>
                    <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm relative overflow-hidden group">
                        ...
                        <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Recebido Confirmado</p>
                        <h3 className="text-4xl font-black text-green-600 tracking-tight">{formatCurrency(stats.totalPaidValue)}</h3>
                        <p className="text-[10px] text-green-500 mt-2 font-black uppercase tracking-tighter">Valores liquidados no período</p>
                    </div>
                    <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm relative overflow-hidden group">
            ...
            {data.length > 0 ? (
              <div className="bg-slate-900 rounded-[3.5rem] border border-slate-800 p-12 flex flex-col md:flex-row items-center justify-between text-center md:text-left space-y-6 md:space-y-0 shadow-2xl relative overflow-hidden group animate-in zoom-in-95">
                ...
                <div className="relative z-10">
                  <h3 className="text-3xl font-black text-white tracking-tight">Base de Dados Completa</h3>
                  <p className="text-slate-400 max-w-md font-medium mt-2 text-lg">Os {filteredData.length.toLocaleString()} registros selecionados estão processados e prontos para auditoria.</p>
                </div>
                ...
                <div className="bg-white rounded-[2.5rem] p-24 text-center border-2 border-dashed border-slate-200 flex flex-col items-center gap-6">
                    ...
                    <div>
                        <h4 className="font-black text-slate-400 uppercase tracking-[0.3em] text-sm">Banco de Dados Offline</h4>
                        <p className="text-slate-300 text-base mt-2 max-w-sm mx-auto">Clique no botão "Baixar Dados do Banco" para iniciar a sincronização completa e habilitar os relatórios.</p>
                    </div>
                </div>
            )}
          </div>
            ...
            <div className="flex-1 overflow-auto custom-scrollbar">
              {isLoading ? (
                <div className="flex flex-col items-center justify-center h-full gap-4 text-slate-400">
                  ...
                  <p className="font-black uppercase text-xs tracking-[0.2em]">Baixando registros: {loadProgress.toLocaleString()}...</p>
                </div>
                ...
                <div className="flex flex-col items-center justify-center h-full text-slate-300 italic py-24">
                  ...
                  <button onClick={() => setViewMode('dashboard')} className="mt-4 text-blue-600 font-black uppercase text-[10px] hover:underline">Voltar e Ajustar Filtros de Carga</button>
                </div>
              ) : (
            ...
            <div className="px-8 py-5 bg-slate-50 border-t border-slate-200 flex justify-between items-center shrink-0">
              <div className="flex items-center gap-3">
                 <div className="bg-white border border-slate-200 px-4 py-2 rounded-xl text-[10px] font-black text-slate-400 uppercase tracking-widest shadow-sm">
                   Total Local: {data.length.toLocaleString()} registros baixados
                 </div>
                 <span className="text-slate-300">|</span>
                 <div className="text-[10px] font-bold text-slate-400 uppercase">Filtro Ativo: {filteredData.length.toLocaleString()} itens</div>
              </div>
              {totalPages > 1 && (
