...
export const ClassesManager: React.FC<ClassesManagerProps> = ({ onBack }) => {
      ...
      if (formData.studioMod1) {
          ...
          if (studio && studio.rentValue) {
              ...
              if (!isNaN(rentValueNum)) {
                setFormData(prev => ({ ...prev, studioRent: rentValueNum }));
              }
          }
      }
  ...
  useEffect(() => {
      const generateCode = (dateStr: string) => {
          ...
          const parts = [location, formData.classCode, formData.extraClass, formData.course, formattedDate];
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-300 h-full flex flex-col pb-20">
                          ...
                          return (
                              <div key={day} className="bg-white min-h-[120px] p-2 hover:bg-slate-50 transition-colors">
                                  <div className="flex justify-between items-start mb-2">
                                      <span className={clsx("w-6 h-6 flex items-center justify-center rounded-full text-xs font-medium", new Date().toISOString().split('T')[0] === currentDateStr ? "bg-purple-600 text-white" : "text-slate-700")}>{day}</span>
                                  </div>
                                  <div className="space-y-1">
                                      {dayClasses.map(cls => {
                                          const isMod1 = cls.dateMod1 === currentDateStr;
                                          return (
                                              <button key={`${cls.id}-${isMod1 ? 'm1' : 'm2'}`} onClick={() => setModalViewerClass(cls)} className={clsx("w-full text-left px-2 py-1.5 rounded border text-[10px] font-medium transition-all shadow-sm mb-1", isMod1 ? "bg-purple-50 text-purple-700 border-purple-200 hover:bg-purple-100" : "bg-orange-50 text-orange-700 border-orange-200 hover:bg-orange-100")}>
      ...
      <div className="flex flex-col lg:flex-row flex-1 bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden h-[calc(100vh-180px)]">
                  ...
                  {isLoadingData ? (
                      ...
                      <div className="divide-y divide-slate-100">
                          {filteredClasses.map(cls => (
                              <div key={cls.id} onClick={() => setSelectedClassId(cls.id)} className={clsx("p-4 cursor-pointer transition-all hover:bg-white relative group", selectedClassId === cls.id ? "bg-white border-l-4 border-l-purple-600 shadow-sm z-10" : "bg-transparent border-l-4 border-l-transparent text-slate-600")}>
                                  ...
                                  <h3 className={clsx("font-bold text-sm mb-1 leading-snug", selectedClassId === cls.id ? "text-purple-900" : "text-slate-800")}>{cls.course}</h3>
          ...
          <div className="hidden lg:flex flex-col flex-1 bg-slate-50 overflow-hidden relative">
              {selectedClass ? (
                  <ClassStudentsViewer classItem={selectedClass} onClose={() => setSelectedClassId(null)} variant="embedded" canTakeAttendance={true} />
              ) : (
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            <div>
                                ...
                                <select className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm bg-white" value={formData.state} onChange={e => { handleInputChange('state', e.target.value); handleInputChange('city', ''); handleInputChange('studioMod1', ''); }}>
                            ...
                            <div>
                                ...
                                <select className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm bg-white disabled:bg-slate-100" value={formData.city} onChange={e => { handleInputChange('city', e.target.value); handleInputChange('studioMod1', ''); }} disabled={!formData.state || isLoadingCities}>
                            ...
                            <div>
                                ...
                                <input type="text" className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm" placeholder="Ex: 105" value={formData.classCode} onChange={e => handleInputChange('classCode', e.target.value)} />
                            </div>
                            <div>
                                <label className="block text-xs font-semibold text-slate-600 mb-1">Turma EXTRA</label>
                                <input type="text" className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm" placeholder="Sim/Não/Detalhes" value={formData.extraClass} onChange={e => handleInputChange('extraClass', e.target.value)} />
                            </div>
                            <div className="lg:col-span-2">
                                <label className="block text-xs font-semibold text-slate-600 mb-1">Curso</label>
                                <select className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm bg-white" value={formData.course} onChange={e => handleInputChange('course', e.target.value)}>
                            ...
                            <div>
                                ...
                                <input type="date" className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm bg-slate-50 text-slate-500" value={formData.createdAt} readOnly />
                            </div>
                        </div>
                    </div>

                    <div>
                        <h4 className="text-sm font-bold text-purple-700 uppercase tracking-wide mb-4 border-b border-slate-100 pb-2 flex items-center gap-2"><CalendarIcon size={16} /> Módulo 1 (Logística)</h4>
                        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            <div className="lg:col-span-4">
                                ...
                                <input type="text" className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm bg-slate-100 text-slate-600 font-mono" value={formData.mod1Code} readOnly placeholder="Gerado automaticamente..." />
                            </div>
                            <div>
                                <label className="block text-xs font-semibold text-slate-600 mb-1">Data do Módulo 1</label>
                                <input type="date" className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm" value={formData.dateMod1} onChange={e => handleInputChange('dateMod1', e.target.value)} />
                            </div>
                            <div>
                                <label className="block text-xs font-semibold text-slate-600 mb-1">Studio MOD I</label>
                                <select className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm bg-white disabled:bg-slate-50" value={formData.studioMod1} onChange={e => handleInputChange('studioMod1', e.target.value)} disabled={!formData.city}>
                            ...
                            <div className="lg:col-span-2">
                                ...
                                <input type="text" className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm bg-slate-100 text-slate-500" value={selectedStudioAddress} readOnly placeholder="Endereço aparecerá aqui..." />
                            </div>
                            <div>
                                <label className="block text-xs font-semibold text-slate-600 mb-1">Instrutor Módulo 1</label>
                                <select className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm bg-white" value={formData.instructorMod1} onChange={e => handleInputChange('instructorMod1', e.target.value)}>
                                    <option value="">Selecione...</option>
                            ...
                            <div>
                                ...
                                <div className="relative"><Plane size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"/><input type="text" className="w-full pl-9 pr-3 py-2 border border-slate-300 rounded-lg text-sm" placeholder="Voo/Horário" value={formData.ticketMod1} onChange={e => handleInputChange('ticketMod1', e.target.value)} /></div>
                            </div>
                            <div>
                                <label className="block text-xs font-semibold text-slate-600 mb-1">MATERIAL</label>
                                <input type="text" placeholder="Apostilas..." className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm" value={formData.material} onChange={e => handleInputChange('material', e.target.value)} />
                            </div>
                            <div>
                                <label className="block text-xs font-semibold text-slate-600 mb-1">PROJETOR, TV, CADEIRAS</label>
                                <input type="text" className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm" value={formData.infrastructure} onChange={e => handleInputChange('infrastructure', e.target.value)} />
                            </div>
                            <div>
                                <label className="block text-xs font-semibold text-slate-600 mb-1">Coffe Módulo 1</label>
                                <div className="relative"><Coffee size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"/><input type="text" className="w-full pl-9 pr-3 py-2 border border-slate-300 rounded-lg text-sm" value={formData.coffeeMod1} onChange={e => handleInputChange('coffeeMod1', e.target.value)} /></div>
                            </div>
                            <div>
                            ...
                            <div className="md:col-span-2">
                                ...
                                <div className="relative"><Bed size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"/><input type="text" className="w-full pl-9 pr-3 py-2 border border-slate-300 rounded-lg text-sm" placeholder="Nome do Hotel" value={formData.hotelMod1} onChange={e => handleInputChange('hotelMod1', e.target.value)} /></div>
                            </div>
                            <div className="md:col-span-2">
                                <label className="block text-xs font-semibold text-slate-600 mb-1">LOCALIZAÇÃO HOTEL MÓDULO 1</label>
                                <div className="relative"><Map size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"/><input type="text" className="w-full pl-9 pr-3 py-2 border border-slate-300 rounded-lg text-sm" placeholder="Endereço / Link" value={formData.hotelLocMod1} onChange={e => handleInputChange('hotelLocMod1', e.target.value)} /></div>
                            </div>
                        </div>
                    </div>

                    <div>
                        <h4 className="text-sm font-bold text-purple-700 uppercase tracking-wide mb-4 border-b border-slate-100 pb-2 flex items-center gap-2"><CalendarIcon size={16} /> Módulo 2 (Logística)</h4>
                        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            <div className="lg:col-span-4">
                                ...
                                <input type="text" className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm bg-slate-100 text-slate-600 font-mono" value={formData.mod2Code} readOnly placeholder="Gerado automaticamente..." />
                            </div>
                            <div>
                                <label className="block text-xs font-semibold text-slate-600 mb-1">Data do Módulo 2</label>
                                <input type="date" className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm" value={formData.dateMod2} onChange={e => handleInputChange('dateMod2', e.target.value)} />
                            </div>
                            <div>
                                <label className="block text-xs font-semibold text-slate-600 mb-1">Instrutor Módulo 2</label>
                                <select className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm bg-white" value={formData.instructorMod2} onChange={e => handleInputChange('instructorMod2', e.target.value)}>
                                    <option value="">Selecione...</option>
                                    {instructorsList.map(i => <option key={i} value={i}>{i}</option>)}
                                </select>
                            </div>
                             <div>
                                ...
                                <div className="relative"><Plane size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"/><input type="text" className="w-full pl-9 pr-3 py-2 border border-slate-300 rounded-lg text-sm" value={formData.ticketMod2} onChange={e => handleInputChange('ticketMod2', e.target.value)} /></div>
                            </div>
                            <div>
                            ...
                            <div className="md:col-span-2">
                                ...
                                <div className="relative"><Map size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"/><input type="text" className="w-full pl-9 pr-3 py-2 border border-slate-300 rounded-lg text-sm" placeholder="Endereço / Link" value={formData.hotelLocMod2} onChange={e => handleInputChange('hotelLocMod2', e.target.value)} /></div>
                            </div>
                            <div>
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                             <h4 className="text-sm font-bold text-purple-700 uppercase tracking-wide mb-4 border-b border-slate-100 pb-2 flex items-center gap-2"><DollarSign size={16} /> Financeiro & Admin</h4>
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-xs font-semibold text-slate-600 mb-1">ALUGUEL DO STUDIO</label>
                                    <input type="number" className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm" placeholder="0.00" value={formData.studioRent || ''} onChange={e => handleInputChange('studioRent', parseFloat(e.target.value) || 0)} />
                                </div>
                            </div>
                        </div>
                        <div>
                            <h4 className="text-sm font-bold text-purple-700 uppercase tracking-wide mb-4 border-b border-slate-100 pb-2 flex items-center gap-2"><CheckSquare size={16} /> Checklist de Prontidão</h4>
                            <div className="space-y-3 bg-slate-50 p-4 rounded-xl border border-slate-200">
                                <label className="flex items-center justify-between cursor-pointer p-2 hover:bg-white rounded transition-colors"><span className="text-sm font-medium text-slate-700">Tudo pronto para o Curso?</span><input type="checkbox" className="w-5 h-5 rounded text-purple-600 focus:ring-purple-500" checked={formData.isReady} onChange={e => handleInputChange('isReady', e.target.checked)} /></label>
                                <div className="h-px bg-slate-200"></div>
                                <label className="flex items-center justify-between cursor-pointer p-2 hover:bg-white rounded transition-colors"><span className="text-sm font-medium text-slate-700">Cadastrado no site</span><input type="checkbox" className="w-5 h-5 rounded text-purple-600 focus:ring-purple-500" checked={formData.onSite} onChange={e => handleInputChange('onSite', e.target.checked)} /></label>
                                <div className="h-px bg-slate-200"></div>
                                <label className="flex items-center justify-between cursor-pointer p-2 hover:bg-white rounded transition-colors"><span className="text-sm font-medium text-slate-700">Cadastrado no CRM</span><input type="checkbox" className="w-5 h-5 rounded text-purple-600 focus:ring-purple-500" checked={formData.onCRM} onChange={e => handleInputChange('onCRM', e.target.checked)} /></label>
                            </div>
                        </div>
                     </div>
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <textarea className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm h-32 resize-none" value={formData.observations} onChange={e => handleInputChange('observations', e.target.value)}></textarea>
                        </div>
                        <div>
                            <label className="block text-xs font-semibold text-slate-600 mb-1">Anexos</label>
                            <div className="border-2 border-dashed border-slate-300 rounded-lg h-32 flex flex-col items-center justify-center text-slate-400 bg-slate-50 hover:bg-slate-100 transition-colors cursor-pointer"><Paperclip size={24} className="mb-2" /><span className="text-sm">Arraste arquivos ou clique aqui</span><input type="file" className="hidden" multiple /></div>
                        </div>
                     </div>
                </div>
