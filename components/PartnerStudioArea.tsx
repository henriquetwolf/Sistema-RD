...
export const PartnerStudioArea: React.FC<PartnerStudioAreaProps> = ({ studio, onLogout }) => {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const { data } = await appBackend.client.from('crm_classes').select('*').eq('studio_mod_1', studio.fantasyName).order('date_mod_1', { ascending: true });
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full p-4 md:p-6">
        <div className="animate-in fade-in duration-300">
            {activeTab === 'support' ? (
                <SupportChannel userId={studio.email} userName={studio.fantasyName} userType="studio" />
            ) : activeTab === 'classes' ? (
                <div className="grid grid-cols-1 gap-4">
                    {classes.map(cls => (
                         <div key={cls.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex justify-between items-center">
                            <div><h3 className="font-bold text-slate-800">{cls.course}</h3><p className="text-xs text-slate-500">{cls.city}/{cls.state}</p></div>
                            <div className="text-right"><p className="text-[10px] font-black text-teal-600 uppercase">Mod 1</p><p className="text-sm font-bold text-slate-700">{new Date(cls.date_mod_1).toLocaleDateString()}</p></div>
                         </div>
                    ))}
                </div>
            ) : (
