...
export const StudentArea: React.FC<StudentAreaProps> = ({ student, onLogout, logoUrl }) => {
    const [activeTab, setActiveTab] = useState<'classes' | 'online_courses' | 'certificates' | 'events' | 'contracts' | 'purchases' | 'my_data' | 'learning_profile' | 'ai_tutor' | 'franchise_presentation' | 'studio_digital' | 'digital_workbook' | 'gamification' | 'financeiro'>('classes');
    ...
    const fetchFinanceiro = async () => {
        ...
        try {
            const docs: string[] = [];
            if (student.cpf?.trim()) docs.push(student.cpf.trim());

            const names = [student.name?.trim()].filter(Boolean) as string[];
            ...
            if (error) {
                console.error('[Financeiro Aluno] Erro RPC:', error);
                setFinReceivables([]);
                setFinPayables([]);
                return;
            }
            ...
            <header className="bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-30 shadow-sm">
                <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-6">
                        <img src={logoUrl || VOLL_LOGO_BASE64} alt="VOLL" className="h-8 max-w-[150px] object-contain" />
                ...
                <div className="animate-in fade-in slide-in-from-bottom-2 duration-500 pb-20">
                    ...
                    {activeTab === 'financeiro' && (
                        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500 pb-20">
                            ...
                            ) : !student.cpf?.replace(/\D/g, '') && finReceivables.length === 0 && finPayables.length === 0 ? (
                                ...
                                <>
                                    {(finReceivables.length > 0 || finPayables.length > 0) && (
                                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                            <div className="bg-red-50 rounded-2xl p-5 border border-red-100">
                                                ...
                                                <p className="text-xl font-black text-red-700 mt-1">{formatCurrency(finReceivables.reduce((s: number, r: any) => s + (Number(r.valor) || 0), 0))}</p>
                                            </div>
                                            <div className="bg-blue-50 rounded-2xl p-5 border border-blue-100">
                                                <p className="text-[10px] font-black text-blue-600 uppercase">Já Pago</p>
                                                <p className="text-xl font-black text-blue-700 mt-1">{formatCurrency(finReceivables.reduce((s: number, r: any) => s + (Number(r.valor_pago) || 0), 0))}</p>
                                            </div>
                                            <div className="bg-emerald-50 rounded-2xl p-5 border border-emerald-100">
                                                <p className="text-[10px] font-black text-emerald-600 uppercase">A Receber (Total)</p>
                                                <p className="text-xl font-black text-emerald-700 mt-1">{formatCurrency(finPayables.reduce((s: number, p: any) => s + (Number(p.valor) || 0), 0))}</p>
                                            </div>
                                            <div className="bg-amber-50 rounded-2xl p-5 border border-amber-100">
                                                <p className="text-[10px] font-black text-amber-600 uppercase">Em Aberto (Pagar)</p>
                                                <p className="text-xl font-black text-amber-700 mt-1">{formatCurrency(finReceivables.reduce((s: number, r: any) => s + ((Number(r.valor) || 0) - (Number(r.valor_pago) || 0)), 0))}</p>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    <div>
                                        <h3 className="text-sm font-black text-slate-700 uppercase tracking-widest mb-4 flex items-center gap-2">
                                            ...
                                            <span className="text-[10px] font-bold bg-red-50 text-red-600 px-2 py-0.5 rounded-full border border-red-200">{filteredFinReceivables.length}</span>
                                        </h3>
                                        {filteredFinReceivables.length === 0 ? (
                                            ...
                                            <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                                                <div className="overflow-x-auto">
                                                    <table className="w-full text-sm">
                                                        ...
                                                        <tbody className="divide-y divide-slate-100">
                                                            {filteredFinReceivables.map((r: any) => {
                                                                ...
                                                                return (
                                                                    <tr key={r.id} className="hover:bg-slate-50">
                                                                        <td className="px-4 py-3"><p className="font-bold text-slate-700 truncate max-w-[250px]">{r.descricao || '--'}</p>{r.numero_documento && <p className="text-[10px] text-slate-400 mt-0.5">Doc: {r.numero_documento}</p>}</td>
                                                                        <td className="px-4 py-3 text-slate-500 text-xs">{r.categoria_nome || '--'}</td>
                                                                        <td className="px-4 py-3 text-center text-xs font-bold text-slate-600">{r.parcela_numero && r.total_parcelas ? `${r.parcela_numero}/${r.total_parcelas}` : '--'}</td>
                                                                        <td className="px-4 py-3 text-xs"><span className={clsx("font-bold", isOverdue ? "text-red-600" : "text-slate-600")}>{r.data_vencimento ? new Date(r.data_vencimento + 'T00:00:00').toLocaleDateString('pt-BR') : '--'}</span></td>
                                                                        ...
                                                                        <td className="px-4 py-3 text-center"><span className={clsx("text-[9px] font-black px-2.5 py-1 rounded-full border uppercase", isPaid ? "bg-emerald-50 text-emerald-700 border-emerald-200" : isOverdue ? "bg-red-50 text-red-700 border-red-200" : "bg-amber-50 text-amber-700 border-amber-200")}>{isPaid ? 'PAGO' : isOverdue ? 'VENCIDO' : 'PENDENTE'}</span></td>
                                                                    </tr>
                                                                );
                                                            })}
                                                        </tbody>
                                                ...
                                                <div className="px-4 py-3 bg-slate-50 border-t flex items-center justify-between text-xs font-bold text-slate-500">
                                                    <span>Total: {filteredFinReceivables.length} registro(s)</span>
                                                    <div className="flex gap-4">
                                                        <span>Valor total: <span className="text-slate-800">{formatCurrency(filteredFinReceivables.reduce((s: number, r: any) => s + (Number(r.valor) || 0), 0))}</span></span>
                                                        <span>Total pago: <span className="text-emerald-600">{formatCurrency(filteredFinReceivables.reduce((s: number, r: any) => s + (Number(r.valor_pago) || 0), 0))}</span></span>
                                                    </div>
                                                </div>
                                            </div>
                                        )}
                                    ...
                                    <div>
                                        <h3 className="text-sm font-black text-slate-700 uppercase tracking-widest mb-4 flex items-center gap-2">
                                            ...
                                            <span className="text-[10px] font-bold bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded-full border border-emerald-200">{filteredFinPayables.length}</span>
                                        </h3>
                                        {filteredFinPayables.length === 0 ? (
                                            ...
                                            <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                                                            ...
                                                            {filteredFinPayables.map((p: any) => {
                                                                ...
                                                                return (
                                                                    <tr key={p.id} className="hover:bg-slate-50">
                                                                        <td className="px-4 py-3"><p className="font-bold text-slate-700 truncate max-w-[250px]">{p.descricao || '--'}</p>{p.numero_documento && <p className="text-[10px] text-slate-400 mt-0.5">Doc: {p.numero_documento}</p>}</td>
                                                                        <td className="px-4 py-3 text-slate-500 text-xs">{p.categoria_nome || '--'}</td>
                                                                        <td className="px-4 py-3 text-center text-xs font-bold text-slate-600">{p.parcela_numero && p.total_parcelas ? `${p.parcela_numero}/${p.total_parcelas}` : '--'}</td>
                                                                        <td className="px-4 py-3 text-xs"><span className={clsx("font-bold", isOverdue ? "text-red-600" : "text-slate-600")}>{p.data_vencimento ? new Date(p.data_vencimento + 'T00:00:00').toLocaleDateString('pt-BR') : '--'}</span></td>
                                                                        ...
                                                                        <td className="px-4 py-3 text-center"><span className={clsx("text-[9px] font-black px-2.5 py-1 rounded-full border uppercase", isPaid ? "bg-emerald-50 text-emerald-700 border-emerald-200" : isOverdue ? "bg-red-50 text-red-700 border-red-200" : "bg-amber-50 text-amber-700 border-amber-200")}>{isPaid ? 'RECEBIDO' : isOverdue ? 'VENCIDO' : 'PENDENTE'}</span></td>
                                                                    </tr>
                                                                );
                                                            })}
                                                        </tbody>
                                                ...
                                                <div className="px-4 py-3 bg-slate-50 border-t flex items-center justify-between text-xs font-bold text-slate-500">
                                                    <span>Total: {filteredFinPayables.length} registro(s)</span>
                                                    <div className="flex gap-4">
                                                        <span>Valor total: <span className="text-slate-800">{formatCurrency(filteredFinPayables.reduce((s: number, p: any) => s + (Number(p.valor) || 0), 0))}</span></span>
                                                        <span>Total recebido: <span className="text-emerald-600">{formatCurrency(filteredFinPayables.reduce((s: number, p: any) => s + (Number(p.valor_pago) || 0), 0))}</span></span>
                                                    </div>
                                                </div>
                                            </div>
                                        )}
