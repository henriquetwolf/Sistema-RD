...
import { 
  ...
  ArrowDownToLine, Monitor, BarChart
} from 'lucide-react';
import { 
...
export const ContaAzulManager: React.FC = () => {
  const [activeSubTab, setActiveSubTab] = useState<'internal' | 'powerbi'>('internal');
    ...
    <div className="flex flex-col h-full animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4 shrink-0">
        <div className="flex items-center gap-4">
            ...
            {loadProgress > 0 && activeSubTab === 'internal' && (
                <div className={clsx(
        ...
        <div className="flex items-center gap-4">
          <div className="flex bg-slate-100 p-1 rounded-xl shadow-inner shrink-0">
              <button onClick={() => setActiveSubTab('internal')} className={clsx("px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all flex items-center gap-2", activeSubTab === 'internal' ? "bg-white text-blue-700 shadow-sm" : "text-slate-500 hover:text-slate-700")}>
                  <BarChart size={16}/> Auditoria Interna
              </button>
              <button onClick={() => setActiveSubTab('powerbi')} className={clsx("px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all flex items-center gap-2", activeSubTab === 'powerbi' ? "bg-white text-blue-700 shadow-sm" : "text-slate-500 hover:text-slate-700")}>
                  <Monitor size={16}/> Power BI
              </button>
          </div>
        ...
        {activeSubTab === 'powerbi' ? (
          <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-xl overflow-hidden flex flex-col h-[700px] animate-in slide-in-from-right-4 duration-500">
            <div className="bg-slate-50 border-b border-slate-200 px-8 py-4 flex items-center justify-between">
                ...
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Dados em tempo real • VOLL Group</span>
            </div>
            ...
            {data.length > 0 && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in slide-in-from-bottom-4 delay-150">
                    ...
                    <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm h-[380px] flex flex-col">
                        ...
                        <div className="flex-1">
                            <ResponsiveContainer width="100%" height="100%">
                                <ReBarChart data={analytics.charts.categoryChart} layout="vertical" margin={{ left: 20 }}>
