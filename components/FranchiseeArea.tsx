  ...
  const fetchFinanceiro = async () => {
    ...
    try {
      ...
      if (cleanCpf.length >= 11) {
        ...
        if (cleanCpf.length === 14) docVariants.add(cleanCpf.replace(/(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, '$1.$2.$3/$4-$5'));
      }
      ...
      const nameVariants = [...new Set([franchise?.company_name?.trim(), franchise?.franchisee_name?.trim()].filter(Boolean))] as string[];
