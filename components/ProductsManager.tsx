...
export const ProductsManager: React.FC<ProductsManagerProps> = ({ onBack }) => {
  ...
  const handleDeleteModule = async (id: string) => {
      if (!window.confirm("Excluir módulo e todas as aulas dele?")) return;
              ...
              <div className="bg-white border-b border-slate-200 px-8 py-5 flex items-center justify-between shrink-0 shadow-sm z-10">
                  <div className="flex items-center gap-4">
                      ...
                      <div>
                          ...
                          <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">Editor de Conteúdo LMS</p>
                      </div>
                  </div>
                  <div className="flex gap-3">
                      <button onClick={handleAddModule} className="bg-teal-600 hover:bg-teal-700 text-white px-5 py-2 rounded-xl font-black text-sm flex items-center gap-2 shadow-lg shadow-teal-600/20 active:scale-95 transition-all">
                      ...
                      <div className="max-w-4xl mx-auto space-y-4">
                          {modules.length === 0 ? (
                              <div className="text-center py-20 bg-white border-2 border-dashed border-slate-200 rounded-[2rem] text-slate-400">
                                  ...
                                  <p className="text-sm mt-1">Comece adicionando o primeiro módulo do curso.</p>
                              </div>
                          ) : (
                              modules.map(mod => (
                                  <div key={mod.id} className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden group">
                                      <div className="px-6 py-4 flex items-center justify-between hover:bg-slate-50 cursor-pointer" onClick={() => toggleModule(mod.id)}>
                                          <div className="flex items-center gap-4">
                                          ...
                                          <div className="flex items-center gap-2">
                                              <button onClick={(e) => { e.stopPropagation(); handleAddLesson(mod.id); }} className="px-3 py-1 bg-teal-50 text-teal-700 rounded-lg text-[10px] font-black uppercase tracking-widest border border-teal-100 hover:bg-teal-100 transition-colors">Add Aula</button>
                                              <button onClick={(e) => { e.stopPropagation(); handleDeleteModule(mod.id); }} className="p-1.5 text-slate-300 hover:text-red-500"><Trash2 size={16}/></button>
                                      ...
                                      {expandedModules.includes(mod.id) && (
                                          <div className="p-4 bg-slate-50/50 border-t border-slate-100 space-y-2 animate-in slide-in-from-top-2">
                                                          ...
                                                          <div className="flex items-center gap-4 flex-1 min-w-0">
                                                              ...
                                                              <div className="flex-1 min-w-0">
                                                                  ...
                                                                  <div className="flex items-center gap-3 mt-0.5">
                                                                      <span className="text-[10px] font-black text-teal-600 uppercase tracking-widest">Aula {lesson.order}</span>
                                                                      <span className="text-[10px] text-slate-400 truncate max-w-[200px]">{lesson.videoUrl || 'Sem vídeo'}</span>
                                                                  </div>
                                                              </div>
                                                          </div>
                                                          <button onClick={() => handleDeleteLesson(mod.id, lesson.id)} className="p-2 text-slate-300 hover:text-red-500 opacity-0 group-hover/les:opacity-100 transition-opacity"><Trash2 size={16}/></button>
                                                      </div>
                                                  ))
                                              ) : (
                                                  <div className="text-center py-6 text-slate-300 text-xs italic">Nenhuma aula neste módulo.</div>
                                              )}
                                          </div>
                                      )}
                                  </div>
        ...
        {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filtered.map(product => (
                    <div key={product.id} className="bg-white rounded-[2rem] border border-slate-200 shadow-sm hover:shadow-xl hover:border-indigo-200 transition-all overflow-hidden flex flex-col group">
                        <div className="h-32 bg-slate-50 relative overflow-hidden shrink-0">
                            ...
                            <div className="absolute top-4 left-4">
                                <span className={clsx("text-[9px] font-black px-2 py-1 rounded-full uppercase tracking-widest border shadow-sm", product.status === 'active' ? "bg-green-50 text-white border-transparent" : "bg-white text-slate-500 border-slate-200")}>
                        ...
                        <div className="p-6 flex-1 flex flex-col">
                                ...
                                <div className="relative">
                                    ...
                                    {activeMenuId === product.id && (
                                        <div className="absolute right-0 top-8 w-40 bg-white rounded-xl shadow-xl border border-slate-200 z-50 animate-in fade-in zoom-in-95 duration-100 overflow-hidden">
                                            <button onClick={() => handleEdit(product)} className="w-full text-left px-4 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50 flex items-center gap-2">
                                                <Edit2 size={14} /> Editar Produto
                                            </button>
                                            {product.category === 'Curso Online' && (
                                                <button onClick={() => openCourseBuilder(product)} className="w-full text-left px-4 py-2.5 text-xs font-bold text-indigo-600 hover:bg-indigo-50 flex items-center gap-2">
                                                    <ListTodo size={14} /> Editar Grade (LMS)
                                                </button>
                                            )}
                                            <div className="h-px bg-slate-100"></div>
                                            <button onClick={() => handleDelete(product.id)} className="w-full text-left px-4 py-2.5 text-xs font-bold text-red-600 hover:bg-red-50 flex items-center gap-2">
                            ...
                            <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">
                                <Tag size={10} className="text-indigo-400"/> {product.category}
                            </div>
                            ...
                            <div className="flex items-center justify-between pt-4 border-t border-slate-100 mt-auto">
                                <span className="font-black text-slate-800 text-xl">{formatCurrency(product.price)}</span>
                                <div className="flex gap-2">
                                    {product.category === 'Curso Online' && (
                                        <button onClick={() => openCourseBuilder(product)} className="p-2 bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-600 hover:text-white transition-all shadow-sm" title="Grade do Curso">
                                            <ListTodo size={18} />
                                        </button>
                                    )}
                                    {product.url && (
                                        <a href={product.url} target="_blank" rel="noreferrer" className="p-2 bg-slate-100 text-slate-600 rounded-xl hover:bg-slate-800 hover:text-white transition-all shadow-sm" title="Página de Vendas">
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
                <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl animate-in zoom-in-95 flex flex-col max-h-[90vh]">
                    <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-slate-50 rounded-t-3xl shrink-0">
                        <div className="flex items-center gap-4">
                            <div className="p-3 bg-indigo-100 rounded-2xl text-indigo-600"><ShoppingBag size={24}/></div>
                            <div>
                                <h3 className="text-xl font-black text-slate-800 leading-tight">{formData.id ? 'Editar Produto' : 'Novo Produto Digital'}</h3>
                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em]">Configurações de Oferta</p>
                            </div>
                        </div>
                        <button onClick={() => setShowModal(false)} className="text-slate-400 hover:text-slate-600 hover:bg-slate-200 rounded-full p-2 transition-all"><X size={24}/></button>
                    </div>
                    ...
                    <div className="p-8 overflow-y-auto custom-scrollbar space-y-6 flex-1">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="md:col-span-2">
                                ...
                                <input 
                                    type="text" 
                                    className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold focus:bg-white focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all" 
                                    value={formData.name}
                                    onChange={e => setFormData({ ...formData, name: e.target.value })}
                                    placeholder="Ex: Formação Completa em Pilates Online"
                                />
                            </div>
                            ...
                            <div>
                                ...
                                <select 
                                    className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold bg-white focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all"
                            ...
                            <div>
                                ...
                                <input 
                                    type="text" 
                                    list="platforms"
                                    className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold focus:bg-white outline-none transition-all" 
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input 
                                        type="number" 
                                        className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-black text-emerald-600 outline-none" 
                            ...
                            <div>
                                ...
                                <select 
                                    className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold bg-white"
                            ...
                            <div className="md:col-span-2">
                                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Thumbnail (Capa URL)</label>
                                <div className="relative">
                                    <ImageIcon className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" size={16} />
                                    <input 
                                        type="text" 
                                        className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-mono text-slate-500" 
                            ...
                            <div className="md:col-span-2">
                                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1 flex items-center gap-1">
                                    <Award size={12} className="text-amber-500" /> Modelo de Certificado Associado
                                </label>
                                <select 
                                    className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold bg-white"
                            ...
                            <div className="md:col-span-2">
                                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Link de Venda / Página Externa</label>
                                <div className="relative">
                                    <Globe size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" />
                                    <input 
                                        type="text" 
                                        className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm" 
                                        value={formData.url}
                            ...
                            <div className="md:col-span-2">
                                ...
                                <textarea 
                                    className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-[1.5rem] text-sm h-28 resize-none focus:bg-white outline-none leading-relaxed"
                                    value={formData.description}
                                    onChange={e => setFormData({ ...formData, description: e.target.value })}
                                    placeholder="Explique o que o aluno terá acesso ao adquirir este produto..."
                                ></textarea>
                            </div>
                        </div>
                    </div>

                    <div className="px-8 py-5 bg-slate-50 flex justify-between items-center border-t border-slate-100 rounded-b-3xl shrink-0">
                        <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase">
                            <Info size={14}/> ID: {formData.id || 'NOVO'}
                        </div>
                        <div className="flex gap-3">
                            <button onClick={() => setShowModal(false)} className="px-6 py-2.5 text-slate-500 font-black text-xs uppercase tracking-widest hover:bg-slate-200 rounded-xl transition-all">Cancelar</button>
                            <button onClick={handleSave} disabled={isSaving} className="px-10 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl shadow-indigo-600/20 transition-all flex items-center gap-2 active:scale-95 disabled:opacity-50">
