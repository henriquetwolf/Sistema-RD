...
-- 6. ATUALIZAÇÃO CRM DEALS (Número Automático de 12 Dígitos)
...
-- Bloco para garantir que a sequência comece em 100.000.000.001 (12 dígitos)
DO $$
DECLARE
    seq_name text;
    curr_val bigint;
BEGIN
    SELECT pg_get_serial_sequence('public.crm_deals', 'deal_number') INTO seq_name;
    IF seq_name IS NOT NULL THEN
        EXECUTE 'SELECT last_value FROM ' || seq_name INTO curr_val;
        -- Se o valor atual for menor que 12 dígitos, reinicia a sequência
        IF curr_val < 100000000000 THEN
            EXECUTE 'ALTER SEQUENCE ' || seq_name || ' RESTART WITH 100000000001';
        END IF;
    END IF;
