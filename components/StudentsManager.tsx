...
export const StudentsManager: React.FC<StudentsManagerProps> = ({ onBack }) => {
  ...
  const fetchData = async () => {
    ...
    try {
        ...
        setStudents(data.map((s: any) => ({ ...s, student_access_enabled: s.student_access_enabled !== false })));
  ...
  const toggleAccess = async (courseId: string) => {
      const isGranting = !studentAccessedIds.includes(courseId);
      if (isGranting) {
          setStudentAccessedIds([...studentAccessedIds, courseId]);
      } else {
          setStudentAccessedIds(studentAccessedIds.filter(id => id !== courseId));
      }
  };
  ...
  const saveAccessChanges = async () => {
      ...
      try {
          // Simplificado: Limpa e reinsere para o deal_id
          ...
          if (studentAccessedIds.length > 0) {
              const inserts = studentAccessedIds.map(cid => ({ student_deal_id: unlockModalStudent.id, course_id: cid }));
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden overflow-x-auto min-h-[400px]">
            {isLoading ? <div className="flex justify-center items-center h-64"><Loader2 size={32} className="animate-spin text-teal-600" /></div> : (
                <table className="w-full text-left text-sm text-slate-600">
                    <thead className="bg-slate-50 text-xs uppercase font-semibold text-slate-500"><tr><th className="px-6 py-4">Nome</th><th className="px-6 py-4">Contato</th><th className="px-6 py-4">Produto Base</th><th className="px-6 py-4 text-center">Ações de Conteúdo</th></tr></thead>
                        ...
                        {filtered.map(s => (
                            <tr key={s.id} className="hover:bg-slate-50 transition-colors">
                                ...
                                <td className="px-6 py-4 text-center">
                                    <button 
                                        onClick={() => openUnlockModal(s)}
                                        className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-md hover:bg-indigo-700 active:scale-95 transition-all"
                                    >
                                        <MonitorPlay size={14}/> Liberar Cursos
                                    </button>
                                </td>
                            </tr>
                        ))}
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
                <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl animate-in zoom-in-95 flex flex-col max-h-[85vh]">
                    <div className="px-8 py-6 border-b flex justify-between items-center bg-slate-50">
                        ...
                        <button onClick={() => setUnlockModalStudent(null)} className="p-2 hover:bg-slate-200 rounded-full text-slate-400 transition-colors"><X size={24}/></button>
                    </div>
                    <div className="p-8 overflow-y-auto custom-scrollbar flex-1 space-y-4">
                        ...
                        {onlineCourses.length === 0 ? <p className="text-center py-10 text-slate-400">Nenhum curso online cadastrado no Construtor.</p> : (
                                ...
                                {onlineCourses.map(course => (
                                    <div 
                                        ...
                                        className={clsx(
                                            ...
                                            studentAccessedIds.includes(course.id) ? "bg-indigo-50 border-indigo-500 shadow-sm" : "bg-white border-slate-100 hover:border-indigo-200"
                                        )}
                                    >
                                        <div className="flex items-center gap-4">
                                            <div className={clsx("w-12 h-12 rounded-xl border-2 flex items-center justify-center", studentAccessedIds.includes(course.id) ? "bg-indigo-600 text-white border-white/20" : "bg-slate-50 text-slate-300 border-slate-100 group-hover:text-indigo-400")}>
                                                <MonitorPlay size={24} />
                                            </div>
                                            <div><p className="font-black text-slate-800 text-sm">{course.title}</p><p className="text-[10px] text-slate-400 uppercase font-bold tracking-widest">{course.price ? formatCurrency(course.price) : 'Curso Premium'}</p></div>
                                        </div>
                                        <div className={clsx("w-8 h-8 rounded-full border-2 flex items-center justify-center transition-all", studentAccessedIds.includes(course.id) ? "bg-indigo-500 border-indigo-500 text-white" : "border-slate-200 text-transparent")}>
                                            <Check size={18} />
                                        </div>
                                    </div>
                                ))}
