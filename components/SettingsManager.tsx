...
import { 
    ...
    /* Added MoveRight to imports */
    Play, Pause, Calendar, Smartphone, Link as LinkIcon, ChevronDown, Award, ShoppingBag, Zap, Filter,
    List, ArrowRight, MoveRight, Braces, Sparkles, RefreshCw, BookOpen, Book, ListTodo
} from 'lucide-react';
...
export const SettingsManager: React.FC<SettingsManagerProps> = ({ 
  ...
  const [activeTab, setActiveTab] = useState<'visual' | 'roles' | 'companies' | 'levels' | 'connections' | 'webhooks' | 'database'>('visual');
  ...
  const loadAllData = async () => {
    setIsSaving(true);
    try {
      const [r, c, l, w, p, pr] = await Promise.all([
        appBackend.getRoles(),
        appBackend.getCompanies(),
        appBackend.getInstructorLevels(),
        appBackend.getWebhookTriggers(),
        appBackend.getPipelines(),
        appBackend.client.from('crm_products').select('*')
      ]);
      setRoles(r || []);
      setCompanies(c || []);
  ...
  return (
    <div className="animate-in fade-in space-y-8 pb-20">
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
        <div>
          <h2 className="text-2xl font-black text-slate-800">Configurações Globais</h2>
          <p className="text-sm text-slate-500 font-medium">Administração técnica, permissões e automações do sistema.</p>
        </div>
        <div className="flex bg-white p-1 rounded-2xl border border-slate-200 shadow-sm overflow-x-auto no-scrollbar shrink-0">
          {[
            { id: 'visual', label: 'Visual', icon: Palette },
            { id: 'roles', label: 'Permissões', icon: Lock },
            ...
            <button
              ...
              className={clsx(
                "px-4 py-2 rounded-xl text-xs font-black uppercase tracking-tighter transition-all flex items-center gap-2 whitespace-nowrap",
                activeTab === tab.id ? "bg-slate-800 text-white shadow-lg" : "text-slate-400 hover:text-slate-600 hover:bg-slate-50"
              )}
            >
      ...
      {activeTab === 'visual' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in slide-in-from-left-2">
          ...
          <div className="bg-slate-800 rounded-[2rem] p-10 text-white relative overflow-hidden flex flex-col justify-center">
      ...
      {activeTab === 'webhooks' && (
        <div className="space-y-6 animate-in fade-in slide-in-from-top-2">
          <div className="bg-indigo-900 rounded-[2rem] p-10 text-white relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-8">
        ...
        <div className="bg-white rounded-[2rem] border border-slate-200 p-10 space-y-8 animate-in zoom-in-95">
          ...
          <div className="bg-amber-50 p-6 rounded-2xl border border-amber-200 flex gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-amber-800 leading-relaxed font-medium">Rode este script no editor SQL do seu dashboard Supabase para habilitar o novo **Sistema de Suporte Interno** e garantir que todas as tabelas de CRM e RH tenham os campos necessários para esta version.</p>
            </div>
          </div>
