      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-4">
        ...
        <div className="flex flex-col md:flex-row items-end md:items-center gap-4 pt-3 border-t border-slate-100">
          ...
          {(searchTerm || statusFilter !== 'all' || startDate || endDate) && (
            <button onClick={clearFilters} className="flex items-center gap-1.5 text-xs font-bold text-red-500 hover:text-red-700 transition-colors ml-auto px-3 py-1.5 hover:bg-red-50 rounded-lg"><Eraser size={14} /> Limpar Filtros</button>
          )}
        </div>
      </div>
                    ...
                    <td className="px-6 py-4 text-right relative">
                      ...
                      {activeMenuId === record.id && (
                        <div ref={menuRef} className="absolute right-10 top-0 w-48 bg-white border border-slate-200 rounded-xl shadow-xl z-[100] py-1 animate-in fade-in zoom-in-95 duration-100">
                          <button onClick={() => handleCopyId(record._display_id_cliente || '')} className="w-full text-left px-4 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50 flex items-center gap-2"><Copy size={14} /> Copiar ID Cliente</button>
                          <button onClick={() => openDetails(record)} className="w-full text-left px-4 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50 flex items-center gap-2"><FileText size={14} /> Ver Detalhes</button>
                          <div className="h-px bg-slate-100 my-1"></div>
                          <button onClick={() => setActiveMenuId(null)} className="w-full text-left px-4 py-2 text-xs font-bold text-red-600 hover:bg-red-50 flex items-center gap-2"><X size={14} /> Fechar Menu</button>
                        </div>
                      )}
                    </td>
                  </tr>
                    ...
                    return (
                        <button 
                            ...
                            className={clsx(
                                ...
                                currentPage === pageNum ? "bg-teal-600 text-white shadow-md" : "bg-white border border-slate-200 text-slate-600 hover:border-teal-300"
                            )}
                        >
                            {pageNum}
                        </button>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 flex flex-col max-h-[90vh]">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50 shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div><h3 className="font-bold text-slate-800">Detalhes do Recebimento</h3><p className="text-xs text-slate-500">ID Lançamento: #{selectedDetailRecord.id}</p></div>
              </div>
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-8">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-5 bg-slate-50 rounded-2xl border border-slate-100">
                <div className="flex items-center gap-4">
                  ...
                  <div><h4 className="font-black text-slate-800 text-lg leading-tight">{selectedDetailRecord._display_name}</h4><p className="text-xs text-slate-500 font-mono">ID Cliente: {selectedDetailRecord._display_id_cliente || '--'}</p></div>
                </div>
              ...
              <div className="space-y-4">
                <h5 className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 border-b pb-2"><Hash size={12} /> Metadados Completos</h5>
                <div className="bg-slate-900 rounded-xl p-4 overflow-x-auto"><pre className="text-[10px] text-teal-400 font-mono leading-relaxed whitespace-pre-wrap">{JSON.stringify(Object.fromEntries(Object.entries(selectedDetailRecord).filter(([k]) => !k.startsWith('_'))), null, 2)}</pre></div>
              </div>
            </div>
            <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3 shrink-0">
              <button onClick={() => handleCopyId(selectedDetailRecord._display_id_cliente || '')} className="px-4 py-2 text-teal-600 hover:bg-teal-50 rounded-lg font-bold text-sm transition-colors flex items-center gap-2"><Copy size={16} /> Copiar ID</button>
