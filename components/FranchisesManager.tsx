...
import { 
  ...
  Navigation, AlertTriangle, CheckCircle, Briefcase, Info, ExternalLink, Hash
} from 'lucide-react';
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-300 space-y-6">
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
            <div className="relative">
                ...
                <input type="text" placeholder="Buscar por franqueado ou empresa..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-50 transition-all"/>
            </div>
        </div>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 overflow-y-auto">
                <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl my-8 animate-in zoom-in-95 flex flex-col max-h-[90vh]">
                    ...
                    <div className="p-8 overflow-y-auto custom-scrollbar space-y-10">
                        <section>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                ...
                                <div className="md:col-span-2">
                                    ...
                                    <input type="text" className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm" value={formData.residentialAddress} onChange={e => setFormData({...formData, residentialAddress: e.target.value})} />
                                </div>
                            </div>
                        </section>
                        ...
                        <section>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                ...
                                <div className="md:col-span-4">
                                    ...
                                    <input type="text" className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm" value={formData.commercialAddress} onChange={e => setFormData({...formData, commercialAddress: e.target.value})} />
                                </div>
                            </div>
                        </section>

                        <section>
                            <h4 className="text-sm font-bold text-teal-700 uppercase tracking-wide mb-4 border-b pb-2 flex items-center gap-2"><Navigation size={16}/> Localização e Contrato</h4>
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                <div>
                                ...
                                <div>
                                    ...
                                    <input type="date" className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm" value={formData.contractStartDate} onChange={e => setFormData({...formData, contractStartDate: e.target.value})} />
                                </div>
                                <div className="flex items-end">
                                    <label className="flex items-center gap-2 cursor-pointer p-2 border rounded-lg w-full justify-center bg-slate-50">
                                        <input type="checkbox" checked={formData.hasSignedContract} onChange={e => setFormData({...formData, hasSignedContract: e.target.checked})} className="w-4 h-4 text-teal-600" />
                                        <span className="text-xs font-bold">Contrato Assinado?</span>
                                    </label>
                                </div>
                                <div className="md:col-span-4">
                                    <label className="block text-xs font-semibold text-slate-600 mb-1">Link Pasta Cloud (Docs)</label>
                                    <div className="relative">
                                        <ExternalLink className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={14} />
                                        <input type="text" className="w-full pl-9 pr-3 py-2 border border-slate-300 rounded-lg text-sm text-blue-600" value={formData.franchiseeFolderLink} onChange={e => setFormData({...formData, franchiseeFolderLink: e.target.value})} placeholder="Google Drive / OneDrive" />
                                    </div>
                                </div>
                            </div>
                        </section>
                        ...
                        <section>
                            <h4 className="text-sm font-bold text-slate-700 uppercase tracking-wide mb-4 border-b pb-2 flex items-center gap-2"><FileText size={16}/> Observações Administrativas</h4>
                            <textarea className="w-full border rounded-xl p-4 text-sm h-32 resize-none outline-none focus:ring-2 focus:ring-teal-500" value={formData.observations} onChange={e => setFormData({...formData, observations: e.target.value})} placeholder="Histórico, detalhes de negociação, equipamentos extras..."></textarea>
                        </section>
                    </div>
